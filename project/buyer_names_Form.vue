<template>
  <div :class="$style.root">
    <div v-if="form.id_buyer" :class="$style.item">
      <div :class="$style.label">
        <label for="id_buyer">ID поккупателя</label>
      </div>
      <input :value="form.id_buyer" disabled :class="$style.input" id="id_buyer" placeholder="id_buyer" type="text">
    </div>
...
import { selectItemById, fetchItems, selectItems } from '@/store/buyer_names/selectors';
import Btn from '@/components/Btn/Btn'; // Импортировал Btn

export default {
  name: 'buyer_names_Form',
  components: {
    Btn,
  },
  props: {
  ...
  setup(props, context) {
    ...
    return {
      groupList,
      form,
      isValidForm: computed(() =>  !!(form.buyer_name)),
      onClick: () => {
        context.emit('submit', form);
        router.push({ name: 'Buyer_names' })
      },
    }
